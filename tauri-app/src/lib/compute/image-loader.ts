...
async function drawToContext(blob: Blob): Promise<{
  ...
  const forceHtml =
    ...
    localStorage.getItem('imageLoader.forceHtmlImage') === '1';
  ...
  if (!forceHtml && 'createImageBitmap' in window) {
    try {
      ...
      bitmap.close?.();
      console.warn(
        '[image-loader] ImageBitmap has zero dimensions; falling back to <img>',
        { type, size }
      );
    } catch (err) {
      console.warn(
        '[image-loader] createImageBitmap failed; falling back to <img>',
        err
      );
    }
  } else {
    if (forceHtml) {
      console.info(
        '[image-loader] forceHtmlImage enabled; skipping ImageBitmap',
        { type, size }
      );
    }
  }
...
export async function loadImageDataset(blob: Blob): Promise<ImageDataset> {
  ...
  if (!width || !height) {
    console.error('[image-loader] decoded zero-sized image', {
