...
depends_on: [AI-EPIC-021]
parent_epic: [[AI-EPIC-021-accessibility-and-keyboard]]
...
## Summary of Issue #1
HomeView has basic a11y affordances (dropzone roles, overlay labels), but chart views and Exports currently lack explicit roles, ARIA labelling, and keyboard shortcuts. This ticket performs an accessibility and keyboard interaction pass across all views to ensure keyboard-only users can operate the app, that overlays are announced correctly, and that core shortcuts (Upload, Exports) are present and documented. Done means we can tab through all actionable controls, trigger primary actions via the keyboard, and see no obvious a11y warnings in dev tooling.
...
### Design/Approach
- Review existing markup in all views and ensure interactive elements have semantic roles, labels, and visible focus states.
...
- `tauri-app/src/lib/views/ValuesView.svelte`: add roles/labels for value analysis interactions.
...
- [ ] Audit Home, Values, and Exports views for roles/labels/focus and update markup to be keyboard-friendly.
...
- [ ] Perform a manual keyboard-only walkthrough covering load, analyze, navigate views, and trigger exports.

### Acceptance Criteria
**Scenario: Keyboard-only navigation**
GIVEN a user operates the app using only the keyboard
WHEN they tab through Home, Values, and Exports views
